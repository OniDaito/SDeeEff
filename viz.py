...
class SDFView(Frame):
    ...
    def __init__(self, parent, rez=128):
        ...
        self.screen_coords = [] # these two are for a bit of speed
        self.screen_rays = []
        self.near_plane = 0.1
    ...
    def raycast(self, field, device):
        # assume eye at 0, 0, 0

        for idx, coord in enumerate(self.screen_coords):
            dx = coord[0]
            dy = coord[1]
            sx = coord[2]
            sy = coord[3]
        
            ray = torch.tensor(self.screen_rays[idx], dtype=torch.float32, device=device)
            pos = torch.tensor([dx, dy, self.near_plane], dtype=torch.float32, device=device)
            d = field.get_distance(pos, device)
            hit = False
